...
if __name__ == '__main__':
    ...
    th = 2   # int(sys.argv[3])               # search thread num
    switch_alias = False   # bool(int(sys.argv[4]))    # if search with switch alias
    timeout = 60  # int(sys.argv[5])          # search timeout in seconds, permanently if 0

    conn = connections.connect(alias="default", uri=uri,
    ...
    if index_type == "AUTOINDEX":
        search_params = {"metric_type": metric_type,  "params": {"level": 2}}
    else:
